...
class VideoDataset(Dataset):
    ...
    inferred from the respective folder names.
        Args:
            dataset (str): Name of dataset. Defaults to 'setup1'.
            split (str): Determines which folder of the directory the dataset will read from. Defaults to 'train'.
    """

    def __init__(self, dataset='setup1', split='train'):
        self.original_dir = os.path.join('/DATA/rani.1/feb', dataset)
        self.preprocessed_dir = os.path.join('/DATA/rani.1/feb/',dataset, 'preprocessed_' + dataset)
        self.split = split
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            try:
                buffer1 = self.crop(buffer,CLIP_LEN,CROP_SIZE,size2) #for HGD dataset
            except ValueError:
                #for Austim+HMDB51 dataset
                buffer1 = self.crop(buffer,CLIP_LEN,CROP_SIZE,152)
            buffer2 = self.flip(buffer1) #transformation to create the other data of the pair
            buffer1 = self.resize(buffer1)
            buffer2 = self.resize(buffer2)
        buffer1 = self.normalize(buffer1)
        buffer1 = self.to_tensor(buffer1)
        buffer2 = self.normalize(buffer2)
        buffer2 = self.to_tensor(buffer2)
        return [torch.from_numpy(buffer1),torch.from_numpy(buffer2)], torch.from_numpy(label)
    ...
    def process_video(video_name, save_name):
        ...
        while count < frame_count and retaining:
            ...
            if count % extract_frequency == 0:
                ...
                #cv2.imwrite(filename=os.path.join(save_name.split('.')[0], '{}.jpg'.format(str(i))), img=frame)
                i += 1
            count += 1
    ...
    def load_frames(file_dir):
        ...
        for i, frame_name in enumerate(frames):
            total = len(frames)
            skip = int(total/16) #uniform selection of frames, total of 16 as clip len for HGD dataset
            try:
                if not i% skip:
                ...
                skip=int(total/10) #uniform selection of frames, total of 16 as clip len for Autism+ HMDB51 dataset
                if total >= 10:
                    if not i%skip:
                        frame = np.array(cv2.imread(frame_name))
                        buffer.append(frame)
    ...
    def rotation(self,buffer,angle):
        h, w = buffer[0].shape[:2]
        angle = int(random.uniform(-angle, angle))
        M = cv2.getRotationMatrix2D((int(w/2), int(h/2)), angle, 1)
        for i, frame in enumerate(buffer):
            frame = cv2.warpAffine(buffer[i], M, (w, h))
            buffer[i] = frame
        return buffer
...
def get_labels(dataset='ucf101'):
    labels = []
    with open('/DATA/rani.1/feb/{}/{}_labels.txt'.format(dataset,dataset), 'r') as load_f:
